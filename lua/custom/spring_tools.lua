...
local function makeStsHandler(jdlts_command, title)
  return function(err, result, ctx)
    ...
    }, ctx, function(err, res, _) coroutine.resume(co, err, res) end)
...
function M.on_attach(event)
  ...
  if not client or (client.name ~= 'springboot_ls' and client.name ~= 'jdtls') then return end
  ...
  client.handlers['sts/addClasspathListener'] = function(err, result, ctx)
    ...
    if not jdtls_client then
      ...
      }, function(err, result, ctx) vim.notify('addClasspathListener result: ' .. vim.inspect(result), vim.log.levels.INFO) end, ctx.bufnr)
    end
  ...
  client.handlers['workspace/executeClientCommand'] = function(err, result, ctx)
    ...
    if result.command == 'vscode-spring-boot.ls.start' then
      ...
      if not spls_id then
        ...
        if spls == nil then return vim.lsp.rpc.rpc_response_error(vim.lsp.protocol.ErrorCodes.InternalError) end
        ...
        spls:request('workspace/executeCommand', {
          ...
          if err ~= nil then vim.notify('could not enable classpath listening: ' .. err.message, vim.log.levels.ERROR) end
        end, ctx.bufnr)
    ...
    for _, ac_client in ipairs(clients) do
      ...
      if client.id ~= ac_client.id and (ac_client.name == 'springboot_ls' or ac_client.name == 'jdtls') then
        ...
        ac_client:request('workspace/executeCommand', result, function(err, result, ctx) coroutine.resume(co, err, result) end, ctx.bufnr)
        break
      end
    end
