...
local function add_to_buffer_diagnostics(qfitems, bufnr)
  ...
  for _, item in ipairs(qfitems) do
    ...
    if vim.fn.bufnr(item.bufnr) == bufnr or item.user_data ~= nil and vim.fn.bufname(bufnr) == item.user_data.filename then table.insert(to_insert, item) end
  end
...
local function on_curl_complete(context)
  ...
  return function(job_result)
    if job_result.code ~= 0 then
      vim.schedule(function() vim.notify('HTTP request failed - could not find prs: ' .. job_result.stderr, vim.log.levels.ERROR) end)
      return
    end
    ...
    if data.type ~= nil and data.type == 'error' then
      vim.schedule(function() vim.notify('Error while calling BB api' .. tostring(data), vim.log.levels.ERROR) end)
      return
    end
...
---@param url any
function M.find_bitbucket_pr_comments(url) call_bb_api(url, on_curl_complete {}) end
...
local function autocomplete(argLead, cmdLine, cursorPos) return { 'check', 'find ' } end
