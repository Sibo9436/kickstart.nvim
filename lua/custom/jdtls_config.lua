  ...
  local java = {
    ...
    compile = {
      nullAnalysis = {
        ...
        nonnull = { 'org.jspecify.annotations.NonNull', 'jakarta.validation.constraints.NotNull' },
      },
    },
    diagnostic = {
      nullAnalysis = {
...
function M.on_attach(event)
  ...
  client.commands['java.action.generateToStringPrompt'] = function(cmd, ctx)
    ...
    client:request('java/checkToStringStatus', params, function(err, res, ctx)
      ...
      ui.select(
        {
          prompt = 'To String',
          index_map = function(v) return v.name end,
          custom_virt = function(v) return { { v.isField and 'field' or '', 'Comment' } } end,
        },
        res.fields,
        function(selected_fields)
          client:request('java/generateToString', { context = params, fields = selected_fields }, function(err, res, ctx)
            if err == nil and res ~= nil then vim.lsp.util.apply_workspace_edit(res, 'utf-16') end
          end, bufnr)
        end
      )
    end, bufnr)
      ...
      ui.select(
        ...
        function(selected)
          client:request('java/generateConstructors', {
            ...
            if not err and res ~= nil then vim.lsp.util.apply_workspace_edit(res, 'utf-16') end
          end, bufnr)
        end
      )
    end, bufnr)
      ...
      ui.select(
        {
          ...
          index_map = function(v) return v.field.name end,
        },
        res.delegateFields,
        function(selected)
          ...
          ui.select(
            {
              prompt = cmd.title,
              index_map = function(entry) return entry.field.name .. '.' .. entry.delegateMethod.name end,
  ...
  client.commands['_java.test.askClientForChoice'] = function(cmd, ctx)
    ...
    local resume = function(val) coroutine.resume(co, val) end
    if cmd.arguments[3] then
      ui.select({
        prompt = cmd.arguments[1] --[[@as string]],
        index_map = function(arg) return arg.value or arg.label end,
      }, cmd.arguments[2] --[[@as table]], resume)
    else
      vim.ui.select(cmd.arguments[2] --[[@as table]], {
        prompt = cmd.arguments[1],
        format_item = function(v) return v.value or v.label end,
      }, resume)
    end
    local ret
    ...
    if result and (result.value or result.label) then
      ...
      ret = vim.tbl_map(function(r) return r.value or r.label end, result)
    end
    ---@diagnostic disable-next-line: redundant-return-value
    return ret
  end
  ...
  client.commands['java.action.organizeImports.chooseImports'] = function(cmd, ctx)
    ...
    for _, import_selection in ipairs(import_selections) do
      vim.ui.select(import_selection.candidates, {
        ...
        format_item = function(item) return item.fullyQualifiedName end,
  ...
  client.commands['java.action.applyRefactoringCommand'] = function(cmd, ctx)
    ...
    client:request('java/getRefactorEdit', {
      ...
      if not err and res then vim.lsp.util.apply_workspace_edit(res, 'utf-16') end
    end, bufnr)
  end
      ...
      ui.select(
        {
          ...
          custom_virt = function(o_method) return { { o_method.declaringClass, 'Comment' } } end,
        },
        res.methods,
        function(selected_methods)
          client:request('java/addOverridableMethods', { context = params, overridableMethods = selected_methods }, function(err, res, ctx)
            if res and not err then vim.lsp.util.apply_workspace_edit(res, 'utf-16') end
          end, bufnr)
        end
      )
    end, bufnr)
  ...
  vim.api.nvim_buf_create_user_command(event.buf, 'JavaOrganizeImports', function(args)
    local command = args.fargs[1]
    local uri = nil
    if command == 'file' then
      uri = vim.uri_from_bufnr(event.buf)
    elseif command == 'directory' then
      uri = vim.uri_from_fname(vim.fn.fnamemodify(vim.uri_to_fname(vim.uri_from_bufnr(event.buf)), ':p:h'))
    ...
    client:exec_cmd({
      ...
      if err ~= nil then
        vim.notify('Organize imports failed', vim.log.levels.ERROR)
      else
        vim.notify('Imports organized successfully (you should run a :wa now)', vim.log.levels.INFO)
      end
    end)
  end, {
    nargs = 1,
    complete = function() return { 'file', 'directory', 'project' } end,
  })
