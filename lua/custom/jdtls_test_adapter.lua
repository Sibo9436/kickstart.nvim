...
function M.root(dir) return util.get_client()._client.root_dir end
...
---@param range table
---@return Range4
local function convert_range(range) return { range.start.line, range.start.character, range['end'].line, range['end'].character } end
...
function MessageBuffer:close() self.is_closed = true end
function MessageBuffer:read() return self.buffer end
...
local function build_spec_dap_junit(args)
  if args.strategy ~= 'dap' then vim.notify('launching through java-debug only supported via dap', vim.log.levels.ERROR) end
  local tree = args.tree
  local data = tree:data()
  if data.type ~= 'test' then
...
function M.build_spec(args)
  if args.strategy == 'dap' then return build_spec_dap_junit(args) end
...
function M.results(spec, result, tree)
  ...
  if spec.context.name ~= 'gradle' then return { [tree:data().id] = { status = 'failed' } } end
  local r = {}
  ...
  for _, node in tree:iter_nodes() do
    if node:data().type == 'test' then test_full_names[node:data().id] = node:data().custom_data.fullName end
  end
  ...
  for _, node in tree:iter_nodes() do
    ...
    if value.type == 'file' then
      ...
      local short = nil
      if parse_result[value.id] and parse_result[value.id].ctx then short = parse_result[value.id].ctx.short end
      local errors = nil
      if parse_result[value.id] and parse_result[value.id].ctx then errors = parse_result[value.id].ctx.errors end
      r[value.id] = {
        status = status,
        short = short,
        errors = errors,
